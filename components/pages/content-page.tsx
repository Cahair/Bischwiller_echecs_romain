...
export function ContentPage({ page }: { page: SitePage }) {
  return <main className={styles.page}>
    <header className={`${styles.hero} ${page.image ? "" : styles.heroNoImage}`}>
      ...
      <div className={styles.heroVisual}>{page.image ? <Image src={page.image} alt="" fill priority sizes="(max-width: 800px) 100vw, 50vw" /> : <div className={styles.chessPattern} aria-hidden="true" />}</div>
    </header>
      ...
      <div className={styles.sections}>{page.sections.map((section, index) => <section id={`section-${index + 1}`} className={styles.section} key={section.title} data-reveal>
        ...
        <div className={styles.sectionBody}>
          ...
          {section.links ? <div className={styles.links}>{section.links.map((link) => {
            ...
            return external ? <a href={link.href} key={link.href} target={link.href.startsWith("http") ? "_blank" : undefined} rel="noreferrer"><span>{link.label}</span><b>↗</b></a> : <Link href={link.href} key={link.href}><span>{link.label}</span><b>→</b></Link>;
          })}</div> : null}
        </div>
      </section>)}</div>
    </div>

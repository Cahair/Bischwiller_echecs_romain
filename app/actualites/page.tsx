...
export default async function NewsPage({ searchParams }: { searchParams: Search }) {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.content}>
      <nav className={styles.filters} aria-label="Filtrer les actualités"><Link className={!selected ? styles.active : ""} href="/actualites">Tout</Link>{categories.map((category) => <Link className={selected === category ? styles.active : ""} href={`/actualites?categorie=${encodeURIComponent(category)}`} key={category}>{category}</Link>)}</nav>

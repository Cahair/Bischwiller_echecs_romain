...
export default async function NewsPage({ searchParams }: { searchParams: Search }) {
  ...
  const articles = mergeArticles(generatedArticles, getLocalArticleIndex());
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.content}>
      ...
      <div className={styles.grid}>{visible.map((article) => <article className={styles.card} key={article.id} data-reveal>
        <Link className={"featuredImageFit" in article && article.featuredImageFit === "contain" ? `${styles.image} ${styles.imageContain}` : styles.image} href={`/actualites/${article.slug}`}>{article.featuredImage ? <Image src={article.featuredImage} alt="" fill sizes="(max-width: 560px) 100vw, (max-width: 850px) 50vw, 33vw" /> : null}</Link>

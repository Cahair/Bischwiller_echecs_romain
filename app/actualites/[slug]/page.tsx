...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className={styles.page}><header className={styles.articleHero}>{article.featuredImage && <Image src={article.featuredImage} alt="" fill priority sizes="100vw" />}<div className={styles.shade} /><div className={styles.articleHeading}><span className={styles.eyebrow}>{article.categories.join(" · ") || "Actualités"}</span><h1>{article.title}</h1><p>{date} · {article.author}</p></div></header><article className={styles.prose}><Link className={styles.back} href="/actualites">← Toutes les actualités</Link><ReactMarkdown remarkPlugins={[remarkGfm]}>{article.contentMarkdown}</ReactMarkdown></article><SiteFooter /></main>;
}
